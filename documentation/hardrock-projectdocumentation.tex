...
\section{Model}
Als Ausgangskonzept wird das Hardrock Cafe in Atlantic City verwendet. Wir verwenden auch die realen Grössen dieses Lokals als Ausgangslage.  
\begin{figure}[htp]
	\centering
...
\subsection{Bar}
Die Bar dient als Warteschlange und Puffer für die Gäste, bis Plätze an einem Tisch frei werden. Gäste werden an der Bar bedient und konsumieren Getränke.

In der Realität wäre das Ziel, dass die Bar optimal besetzt ist, weil dies zusätzliche Einnahmen durch Konsumation generiert. Die Wartezeit an der Bar sollte nicht mehr als ein bis zwei Drinks betragen, damit niemand wieder geht, bevor ein Tisch frei wird.
...
\subsection{Tische}
Gäste eines Restaurants essen in der Regel einen bis drei Gänge an einem Tisch. In einem ersten Schritt soll dieser Umstand  vereinfach durch die Dauer der Konsumation abgebildet werden. Auf eine komplexe Menüsimulation in der Granularität der einzelnen Gänge wird verzichtet.
...
In der realen Welt bestehen die Tische aus Vierertischen. Ankommende Gruppen müssen aufgeteilt oder Tische zusammengeschoben werden. Bei weniger als 4 Personen kann es zu leeren Plätzen kommen. Um die Simulation nicht unnötig aufzublähen, soll dies nicht berücksichtigt werden.
Die Anzahl Plätze werden fix gewählt anhand des Beispiels von Atlantic City (250 Plätze).

Nach dem Essen verlassen die Gäste das Hardrock Cafe. Der von ihnen besetzte Tisch wird für neue Gäste freigegeben.
In der Realität kann es durchaus vorkommen, dass Gäste nochmals an die Bar zurückkehren. Wir berücksichtigen diese Option nicht.


\subsection{Küche}
	In der Küche werden die verschiedenen Mahlzeiten gekocht und angerichtet. Die Köche arbeiten gleichzeitig an mehreren Menüs und versuchen, zu einer einzelnen Bestellung gehörende Mahlzeiten möglichst gleichzeitig fertigzustellen.
...
\section{Spezifikationen}
	...
	\subsection{Resultatgrössen}
		Durch die Simulation sollen der \textbf{erzielte Umsatz}, die \textbf{Dauer des Aufenthaltes und Wartezeiten am Tisch}, die \textbf{Gesammtaufenthaltsdauer im Restaurant} und die \textbf{Auslastung der Agents} ermittelt und optimiert werden.
...
\subsection{Zusatzprojekt (optional)}
	...
	\begin{itemize}
		...
		\item Differenzierung der Mahlzeiten: verschieden Menüs mit unterschiedlichen Zubereitungszeiten für die Köche.
...
\chapter{Modellierung}
	\section{Activities}
		...
		Das Bedienpersonal befinden sich in einem geschlossenen Zyklus. Es nimmt dabei am Tisch eine oder mehrere Bestellungen entgegen und reicht diese an die Küche weiter.
...
\begin{landscape}
	\section{Hardrock Flow}
		...
		Gäste warten in einer Queue, bis eine Platzressource zur Belegung an der Bar frei wird. Sind mehr als zehn Gruppen (mean: 20 Personen) in der Queue, so verlassen die Gäste den Ort ohne Restaurantbesuch. Eine belegte Platzressource an der Bar wird freigegeben, sobald eine freie Tischressource belegt werden kann.
		...
		\subsection{Payment}
		Der Vorgang des Bezahlens ist für unser Modell nicht relevant, könnte aber einfach durch eine Verzögerung vor dem Verlassen des Restaurants realisiert werden.
		...
		\begin{figure}[H]
				\includegraphics[page=3,trim=2cm 12cm 12cm 2cm, clip=true,width=1\textwidth]{../model/Modell_v2.pdf}
...
\chapter{Simulation mit Arena}
	...
	\begin{itemize}
		\item In Arena wurde der OptQuest eingesetzt, um das von uns manuell gefundene Optimum zu verifizieren. OptQuest hat unser Resultat bestätigt.   
		\item Anzahl der Köche: Auf den ersten Blick scheint die von uns ermittelte optimale Anzahl der Köche scheint auf den ersten Blick sehr hoch zu sein im Vergleich zum übrigen Personal. Nachfragen bei Fachleuten haben uns bestätigt, dass diese Anzahl sehr stark von den Gegebenheiten eines Restaurants abhängen kann: Anzahl der angebotenen Menus, Art der Zubereitung etc. 
